...
def in_memory_vector_store(mock_embedding_model):
    ...
    This ensures each test is isolated and doesn't leave files on disk.
    """
    # Using ":memory:" creates a temporary, in-memory SQLite database
...
def test_add_single_document(in_memory_vector_store):
    ...
    # Check the main chunks table
    cursor.execute("SELECT content, metadata FROM chunks WHERE id = 1")
    result = cursor.fetchone()
    assert result is not None
    assert result[0] == "This is the first chunk."
    ...
    # Assert that the stored value is a bytes object matching our expected vector
...
def test_add_documents_handles_empty_list(in_memory_vector_store):
    """
    Tests that add_documents handles an empty list gracefully without errors.
    """
...
def test_add_documents_batch_insertion(in_memory_vector_store):
    ...
    """
    # Re-configure the mock to handle generic batch text for this specific test
    mock_model = in_memory_vector_store.embedding_model
    def generic_batch_embed(texts: list[str]):
        return [[(i + 1) * 0.1] + [0.0] * (MODEL_DIMENSION - 1) for i in range(len(texts))]
    mock_model.create_embeddings.side_effect = generic_batch_embed
...
def test_add_documents_raises_error_on_db_failure(mock_embedding_model, mocker):
    """
    Tests that add_documents raises an exception if the database commit fails.
    """
    # Mock the database connection's commit method to raise an error
    mock_conn = MagicMock()
    mock_conn.cursor.return_value.lastrowid = 1 # Mock lastrowid for the loop
